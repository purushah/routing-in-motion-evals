...
start_litellm() {
  (cd infra && .venv/bin/litellm --config "$1" --port 4000 > ../litellm.log 2>&1 &)
  for i in $(seq 1 60); do curl -s -m 2 http://localhost:4000/health/liveliness >/dev/null 2>&1 && return 0; sleep 2; done
  echo "LITELLM FAILED"; tail -3 litellm.log; return 1
}
...
rm -rf runs/rq5-proxy runs/rq5-div-pre runs/rq5-div-post
